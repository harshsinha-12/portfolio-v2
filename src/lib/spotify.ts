...
// Version 3 adds album artwork and duration; older records refresh once.
type SpotifyCache = {
  version: 3;
...
function publicTrack(
  ...
  return { title, artist: artists.join(", "), url, isPlaying, albumArt: track?.album?.images?.[0]?.url, durationMs: track?.duration_ms };
}
...
export async function getSpotifyResponse() {
